...
### Added
- **Facturas SAT reconciliation:** Differences table shows invoice origin (automatic envío/queue, manual FEL, XML import, etc.).
...
### Added
- **Cotización — líneas desde Oferta:** Editable quantity on PRE lines copied from oferta templates; valor final and minimum scale proportionally. New `oferta_template_id` on pre_cotizacion (legacy PREs with description starting with "Oferta" still work).

## [3.119.321-STABLE] - 2026-08-12

### Fixed
- **ComOrdenproduccionTroubleshootingHelper:** Do not filter on `state` when the column is missing (`pre_cotizacion_line`, `quotation_items`); fixes PRE → Cotización tool SQL error.
...
### Fixed
- **Cotización → Factura relacionada:** Hide system-voided (Anulada) invoices from the related-invoice list and exclude them from completed invoiced totals.
...
### Changed
- **Cotización PDF:** Timbre de Prensa lines show `-` in Codigo (PRE reference stays in Descripción only), matching the display view. Product lines unchanged.
...
### Changed
- **Cotización display:** Pre-cotización links removed from the first column; PRE reference (e.g. «— PRE-01121») is shown only in Descripción.
...
### Added
- **Pre-cotización Proveedor Externo:** **Tarjeta de crédito** dropdown (same as pliego pre-cotizaciones) with surcharge rows in the totals footer.
...
### Fixed
- **SQL migration 3.119.290:** Replace `ADD COLUMN IF NOT EXISTS` (MySQL 8.0.12+ only) with `INFORMATION_SCHEMA` checks for MySQL 5.7 / MariaDB.
...
### Changed
- **Administración → Facturas (lista):** Descripción column shows at most 5 lines (full text on hover).
...
### Fixed
- **Retenciones SAT validate:** `updateObject()` pass-by-reference error that caused a 404/error page after Excel upload.
...
### Added
- **Financiero → Listado PRE:** Column **Fecha factura** (from `fel_fecha_emision` / `invoice_date` of the same invoice as Nº factura) on the table and Excel/CSV export.
...
### Added
- **Lista de cotizaciones:** Filters for **Estado** (Creada / Confirmada / Facturada) and **Agente de ventas**.

### Fixed
- **Lista de cotizaciones:** Quotation numbers like `COT-000735` stay on one line in the number column.
...
### Added
- **Comprobante de pago / MT-940:** Under each PA- row, approvers see a sub-row with the matched MT-940 bank movement (reference, date, account, amount) and a green button to approve verification in place; redirects back to the order payment page after approval.
...
### Fixed
- **Cotización — cuotas mismatch warning:** `show_cuotas_mismatch` was never set because PHP array union (`$empty + [...]`) keeps the left-hand `false` value; warning now displays when pre-cotizaciones have different installment counts.
