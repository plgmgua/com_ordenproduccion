...
# Function to get database name and prefix from Joomla configuration
get_db_config() {
    log "Reading Joomla database configuration..."
    
    if [ ! -f "$JOOMLA_ROOT/configuration.php" ]; then
        error "Joomla configuration file not found: $JOOMLA_ROOT/configuration.php"
        exit 1
    fi
    
    # Extract database name and prefix from configuration.php
    DB_NAME=$(grep "public \$db = " "$JOOMLA_ROOT/configuration.php" | sed "s/.*'\([^']*\)'.*/\1/")
    DB_PREFIX=$(grep "public \$dbprefix = " "$JOOMLA_ROOT/configuration.php" | sed "s/.*'\([^']*\)'.*/\1/")
    
    if [ -z "$DB_NAME" ] || [ -z "$DB_PREFIX" ]; then
        error "Could not extract database name or prefix from configuration.php"
        exit 1
    fi
    
    log "Database name: $DB_NAME"
    log "Database prefix: $DB_PREFIX"
...
add_menu_entry() {
    ...
    if [ "$EXISTING_MENU" -gt 0 ]; then
        ...
        mysql -u "$DB_USER" -p"$DB_PASSWORD" "$DB_NAME" -e "
        INSERT INTO \`${DB_PREFIX}menu\` (\`id\`, \`menutype\`, \`title\`, \`alias\`, \`note\`, \`path\`, \`link\`, \`type\`, \`published\`, \`parent_id\`, \`level\`, \`component_id\`, \`checked_out\`, \`checked_out_time\`, \`browserNav\`, \`access\`, \`img\`, \`template_style_id\`, \`params\`, \`lft\`, \`rgt\`, \`home\`, \`language\`, \`client_id\`) VALUES
        (NULL, 'main', 'Orden Produccion', 'orden-produccion', '', 'orden-produccion', 'index.php?option=$COMPONENT_NAME', 'component', 1, 1, 1, $COMPONENT_ID, 0, '0000-00-00 00:00:00', 0, 1, 'class:ordenproduccion', 0, '{}', 0, 0, 0, '*', 1);
        "
