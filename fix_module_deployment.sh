...
# Step 2: Copy module files from component directory
echo "📋 Step 2: Copying module files..."
COMPONENT_MODULE_PATH="$JOOMLA_ROOT/administrator/components/com_ordenproduccion/mod_acciones_produccion"

if [ -d "$COMPONENT_MODULE_PATH" ]; then
    echo "✅ Found module files in component directory"
    sudo cp -r "$COMPONENT_MODULE_PATH/"* "$MODULE_PATH/" || echo "Failed to copy module files"
    echo "✅ Module files copied"
else
    echo "❌ Module files not found in component directory"
    echo "📁 Creating module files manually..."
...
EOF
    ...
    echo "✅ Module files created manually"
fi
...
if [ -d "$COMPONENT_MODULE_PATH/language" ]; then
    sudo cp -r "$COMPONENT_MODULE_PATH/language/"* "$LANGUAGE_PATH/" || echo "Failed to copy language files"
    sudo chown -R www-data:www-data "$LANGUAGE_PATH"
    sudo chmod -R 755 "$LANGUAGE_PATH"
    echo "✅ Language files copied"
else
    echo "⚠️ Language files not found, creating basic ones..."
    
    # Create basic language files
    sudo mkdir -p "$LANGUAGE_PATH/en-GB"
    sudo mkdir -p "$LANGUAGE_PATH/es-ES"
    
    # English language file
    sudo tee "$LANGUAGE_PATH/en-GB/mod_acciones_produccion.ini" > /dev/null << 'EOF'
...
EOF
    ...
    sudo tee "$LANGUAGE_PATH/es-ES/mod_acciones_produccion.ini" > /dev/null << 'EOF'
...
EOF
    ...
    sudo chown -R www-data:www-data "$LANGUAGE_PATH"
    sudo chmod -R 755 "$LANGUAGE_PATH"
    echo "✅ Basic language files created"
fi
