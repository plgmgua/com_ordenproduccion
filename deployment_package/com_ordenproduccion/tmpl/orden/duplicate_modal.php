...
async function submitDuplicateRequest() {
    ...
    try {
        ...
        // Fix URL construction to avoid double question marks
        let finalUrl;
        
        // Clean the endpoint URL first
        let cleanEndpoint = settings.endpoint;
        ...
        if (cleanEndpoint.includes('?')) {
            // Endpoint already has parameters, add with &
            finalUrl = cleanEndpoint + '&' + urlParams;
        } else {
            // Endpoint has no parameters, add with ?
            finalUrl = cleanEndpoint + '?' + urlParams;
        }
        
        // Additional safety: fix any remaining double question marks and malformed parameter separators
        finalUrl = finalUrl
            .replace(/\?([^=&]+)\?/g, '?$1&')  // Fix ?param? -> ?param&
            .replace(/&&+/g, '&')              // Fix multiple & -> single &
            .replace(/\?&/g, '?')              // Fix ?& -> ?
            .replace(/&\?/g, '&')              // Fix &? -> &
