...
(function() {
    ...
    <div class="precotizacion-oferta-facturar mb-3 d-flex flex-wrap align-items-center gap-4">
        ...
        <?php if ($tarjetaCreditoTableOk && !empty($tarjetaCreditoRates)) : ?>
            ...
            <?php else : ?>
            <div class="d-inline-flex align-items-center gap-2 mb-0">
                <span class="small text-muted text-nowrap"><?php echo Text::_('COM_ORDENPRODUCCION_PRE_COTIZACION_TARJETA_LABEL'); ?></span>
                <select class="form-select form-select-sm" style="min-width: 220px; max-width: 280px;" disabled aria-readonly="true">
                    <?php if ($tcCuotasSel > 0) :
                        foreach ($tarjetaCreditoRates as $tcRow) :
                            $cq = (int) ($tcRow->cuotas ?? 0);
                            if ($cq !== $tcCuotasSel) {
                                continue;
                            }
                            $tp = isset($tcRow->tasa_percent) ? (float) $tcRow->tasa_percent : $tcTasa;
                            ?>
                    <option selected><?php echo Text::sprintf('COM_ORDENPRODUCCION_PRE_COTIZACION_TARJETA_OPTION', $cq, number_format($tp, 2)); ?></option>
                            <?php
                            break;
                        endforeach;
                    else : ?>
                    <option selected><?php echo Text::_('COM_ORDENPRODUCCION_PRE_COTIZACION_TARJETA_NONE'); ?></option>
                    <?php endif; ?>
                </select>
            </div>
            <?php endif; ?>
        ...
        <table class="table table-sm w-auto ms-auto">
            <tbody>
                ...
                <tr class="table-primary">
                    <td class="text-end pe-3 fw-bold"><?php echo Text::_('COM_ORDENPRODUCCION_PRE_COTIZACION_TOTAL_CON_TARJETA'); ?></td>
                    <td class="text-end fw-bold">Q <?php echo number_format($totalConTarjeta, 2); ?></td>
                </tr>
                <?php endif; ?>
