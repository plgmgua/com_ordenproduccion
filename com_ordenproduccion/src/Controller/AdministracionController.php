...
class AdministracionController extends BaseController
{
    ...
    public function exportFinancieroExcel()
    {
        ...
        $cols = [
            ...
            $lang->_('COM_ORDENPRODUCCION_FINANCIERO_COL_ORDEN_DATE'),
            ...
            $lang->_('COM_ORDENPRODUCCION_FINANCIERO_COL_MARGEN_TOTAL_SUM'),
        ...
        $fmtOrdenDateExport = $fmtInvoiceDateExport;
        ...
        foreach ($rows as $r) {
            ...
            $outRows[] = [
                $rowPrecotLabel($r),
                $fmtOrdenExport($r),
                $fmtOrdenDateExport($r->financiero_orden_date ?? null),
                $clientX !== '' ? $clientX : '—',
                $facturarLabel($r),
                $ag !== '' ? $ag : '—',
                $invX !== '' ? $invX : '—',
     ...
     * @since   3.119.267
     */
    public function exportMt940TransactionsExcel()
    {
        $app         = Factory::getApplication();
        $user        = Factory::getUser();
        ...
        try {
            /** @var AdministracionModel $model */
            $model = $app->bootComponent('com_ordenproduccion')->getMVCFactory()->createModel('Administracion', 'Site', ['ignore_request' => true]);
            if (!$model || !$model->isMt940TransactionsTableAvailable()) {
        ...
        $cols = [
            ...
            $lang->_('COM_ORDENPRODUCCION_FINANCIERO_MT940_COL_DESCRIPTION'),
            $lang->_('COM_ORDENPRODUCCION_FINANCIERO_MT940_COL_TYPE'),
            $lang->_('COM_ORDENPRODUCCION_FINANCIERO_MT940_EXPORT_COL_CURRENCY'),
    ...
    protected function buildMt940ExportRows(array $rows, string $accountLabel, $lang): array
    {
        $resolveAccount = static function (object $row): string {
            ...
            $name = trim((string) ($row->bank_account_name ?? ''));
        ...
        $typeLabel = static function (string $dc) use ($lang): string {
            if ($dc === 'D') {
                return $lang->_('COM_ORDENPRODUCCION_FINANCIERO_MT940_TYPE_DEBIT');
            }
            if ($dc === 'C') {
                return $lang->_('COM_ORDENPRODUCCION_FINANCIERO_MT940_TYPE_CREDIT');
            }
     ...
     * @since   3.119.267
     */
    protected function exportMt940TransactionsCsv(array $cols, array $rows, string $fileBase, $app): void
    {
        $filename = $fileBase . '-' . date('Y-m-d-His') . '.csv';
        @ob_clean();
        header('Content-Type: text/csv; charset=UTF-8');
        header('Content-Disposition: attachment; filename="' . $filename . '"');
        header('Cache-Control: no-cache, must-revalidate');
    ...
    public function saveInvoiceFacturaPlantilla()
    {
        ...
        try {
            ...
            $model->saveInvoiceFacturaPlantillaSettings([
                'header_izq_html'   => isset($jform['invoice_factura_header_izq_html']) ? (string) $jform['invoice_factura_header_izq_html'] : '',
                'header_der_html'   => isset($jform['invoice_factura_header_der_html']) ? (string) $jform['invoice_factura_header_der_html'] : '',
                'footer_html'       => isset($jform['invoice_factura_footer_html']) ? (string) $jform['invoice_factura_footer_html'] : '',
                'logo_path'         => isset($jform['logo_path']) ? trim((string) $jform['logo_path']) : '',
                'logo_x'            => isset($jform['logo_x']) ? (float) $jform['logo_x'] : 15,
                'logo_y'            => isset($jform['logo_y']) ? (float) $jform['logo_y'] : 15,
                'logo_width'        => isset($jform['logo_width']) ? (float) $jform['logo_width'] : 50,
                'encabezado_izq_x'  => isset($jform['encabezado_izq_x']) ? (float) $jform['encabezado_izq_x'] : 15,
    ...
    /**
     * Save Blink webhook secret and public base URL (Ajustes → Blink test).
     *
     * @return  void
     *
     * @since   3.119.211
     */
    public function saveBlinkWebhookSettings(): void
    {
        $app  = Factory::getApplication();
        $user = Factory::getUser();
        ...
        if (!Session::checkToken('post')) {
            $app->enqueueMessage(Text::_('JINVALID_TOKEN'), 'error');
            $app->redirect(Route::_('index.php?option=com_ordenproduccion&view=administracion&tab=ajustes&subtab=blink_test', false));
        ...
        try {
            $model = $this->getModel('Administracion');
            if (!$model->saveBlinkWebhookSettings($jform)) {
                $app->enqueueMessage(Text::_('COM_ORDENPRODUCCION_AJUSTES_SAVE_ERROR'), 'error');
            } else {
                $app->enqueueMessage(Text::_('COM_ORDENPRODUCCION_BLINK_WEBHOOK_SETTINGS_SAVED'), 'success');
            }
        } catch (\Throwable $e) {
            $app->enqueueMessage(Text::_('COM_ORDENPRODUCCION_AJUSTES_SAVE_ERROR') . ': ' . $e->getMessage(), 'error');
        }

        $app->redirect(Route::_('index.php?option=com_ordenproduccion&view=administracion&tab=ajustes&subtab=blink_test#blink-webhook-section', false));
    }
    ...
    /**
     * Save MT-940 cron secret (Ajustes → MT940 → Importar datos).
     *
     * @return  void
     *
     * @since   3.119.158
     */
    public function saveMt940CronSettings(): void
    {
        $app  = Factory::getApplication();
        $user = Factory::getUser();
        ...
        $rawKey = $app->input->post->getString('mt940_cron_key', '');
        ...
        try {
            ...
            if (!$model) {
                ...
                $model->saveMt940CronKey($rawKey);
                $app->enqueueMessage(Text::_('COM_ORDENPRODUCCION_MT940_CRON_SETTINGS_SAVED'), 'success');
            }
        } catch (\Throwable $e) {
            $app->enqueueMessage(Text::_('COM_ORDENPRODUCCION_AJUSTES_SAVE_ERROR') . ': ' . $e->getMessage(), 'error');
        }
