...
class AdministracionController extends BaseController
{
    ...
    public function saveProveedor()
    {
        ...
        if ($id === false) {
            ...
            $redir  = $editId > 0
                ? Route::_('index.php?option=com_ordenproduccion&view=administracion&tab=proveedores&proveedor_id=' . $editId, false)
                : Route::_('index.php?option=com_ordenproduccion&view=administracion&tab=proveedores&proveedor_id=0', false);
            $app->redirect($redir);
            return;
        }

        $app->enqueueMessage(Text::_('COM_ORDENPRODUCCION_PROVEEDORES_SAVED'), 'success');
        $app->redirect(Route::_('index.php?option=com_ordenproduccion&view=administracion&tab=proveedores&proveedor_id=' . (int) $id, false));
    }
