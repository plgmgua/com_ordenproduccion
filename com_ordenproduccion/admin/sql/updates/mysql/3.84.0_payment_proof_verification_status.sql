...
--   ADD COLUMN `verification_status` VARCHAR(20) NOT NULL DEFAULT 'ingresado'
...
SET @sql = IF(
    ...
    CONCAT('ALTER TABLE `', @tbl, '` ADD COLUMN `verification_status` VARCHAR(20) NOT NULL DEFAULT ''ingresado'' COMMENT ''ingresado=not yet validated; verificado=counts toward client balance'' AFTER `state`'),
    'SELECT 1');
