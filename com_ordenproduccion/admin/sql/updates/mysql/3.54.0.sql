...
-- 2. Migrate existing data (only if ordenes still has payment_proof_id column)
SET @dbname = DATABASE();
SET @ordenes_tbl = (SELECT TABLE_NAME FROM INFORMATION_SCHEMA.TABLES 
    WHERE TABLE_SCHEMA = @dbname AND TABLE_NAME LIKE '%ordenproduccion_ordenes' LIMIT 1);
SET @col_exists = (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS 
    WHERE TABLE_SCHEMA = @dbname AND TABLE_NAME = @ordenes_tbl AND COLUMN_NAME = 'payment_proof_id');
SET @migrate_sql = IF(@col_exists > 0, 
    CONCAT('INSERT INTO joomla_ordenproduccion_payment_orders (payment_proof_id, order_id, amount_applied, created, created_by)
    SELECT o.payment_proof_id, o.id, COALESCE(o.payment_value, 0.00), COALESCE(pp.created, NOW()), COALESCE(pp.created_by, 1)
    FROM `', @ordenes_tbl, '` o
    INNER JOIN joomla_ordenproduccion_payment_proofs pp ON pp.id = o.payment_proof_id
    WHERE o.payment_proof_id IS NOT NULL AND o.payment_proof_id > 0
    AND NOT EXISTS (SELECT 1 FROM joomla_ordenproduccion_payment_orders po WHERE po.payment_proof_id = o.payment_proof_id AND po.order_id = o.id)'),
    'SELECT 1');
